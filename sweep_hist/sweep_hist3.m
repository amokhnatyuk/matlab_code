...
addpath('./sweep_hist');
copyfile([prog_path,'sweep_hist3.m'],dirSave);  % copy this file to dst folder
...
for ii=1:maxBiasSetting+1 
    ...
    hnorm = nout/sum(nout);
    stdev(ii) = std(hnorm);
    sTitle10 = [sTitle9, ' Bias=',num2str(ii-1)];
...
A.stdev=stdev;
A.stamp=stamp;
sTitle14 = [sTitle9,' StDev of pixVal within [',num2str(floor(xmin)),', ',num2str(floor(xmax)),']'];
prefix = sTitle14;

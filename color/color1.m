...
imageWidth = str2num(cell2mat(defaultValue(1)));
gridWidth  = str2num(cell2mat(defaultValue(2)));
%{
...
grayHeight = chipWidth + 2 * gridWidth;
grayImage = zeros(grayHeight, imageWidth,  'uint16');
...
for row = 1 : 4
	...
	for col = 1 : 6
        ...
        if (row == 1)
    		grayImage(row1:row2, col1:col2) = gray_Values(chipNumber);
        end

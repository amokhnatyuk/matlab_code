...
//   MappControl(M_DEFAULT, M_ERROR, M_PRINT_ENABLE);
   ...
   MdigProcess(MilDigitizer, MilGrabBufferList, MilGrabBufferListSize,
                             M_SEQUENCE,  //       M_START, 
                             M_DEFAULT, ProcessingFunction, &UserHookData);
...
//    dynamicData = (UINT16_T *) mxCalloc(imsz+1, sizeof(UINT16_T));
    ...
    for ( z = 0; z < zz; z++ ) {
        dynamicData[z] = (UINT16_T **) mxCalloc(yy, sizeof(UINT16_T *));
        for ( y = 0; y < yy; y++ ) {
            dynamicData[z][y] = (UINT16_T *) mxCalloc(xx, sizeof(UINT16_T ));
        }
    }
...
//        }
        ...
        for (y = 0; y < yy; y++) {
//            dynamicData[z][j] = ((UINT16_T *) buf[z])[j];
            dynamicData[z][y][x] = (UINT16_T) buf[xx*y +x];
        }
        }
        free(buf);
    }    
